...
import type {
  AgentTransferData,
  BalanceData,
  CreateAgentTransferInput,
  CreateWalletTransferInput,
  FondosEGApiSuccess,
  FondosEGClientOptions,
  GetHistoryParams,
...
export class FondosEGClient {
  ...
  async getHistory(
  ...
  ): Promise<HistoryResponseSuccess> {
    ...
    return this.request<HistoryItem[]>(path, {
      method: 'GET',
      signal: options?.signal,
    }) as Promise<HistoryResponseSuccess>;
  }
  ...
  private async request<T>(
    ...
    for (let attempt = 0; attempt <= this.retries; attempt += 1) {
      ...
      try {
        ...
        return payload as FondosEGApiSuccess<T>;
      } catch (error) {
        if (
  ...
  private mergeSignals(timeoutSignal: AbortSignal, userSignal?: AbortSignal): AbortSignal {

...
export default function BalancePage() {
  ...
  useEffect(() => {
    loadData();
  }, [user, isAdmin, isGestor, isClient]);
    ...
    return (
      <div className="space-y-8">
        <section className="rounded-[2rem] border border-border/10 bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.14),transparent_28%),radial-gradient(circle_at_bottom_right,rgba(245,158,11,0.12),transparent_32%),linear-gradient(135deg,rgba(255,255,255,0.86),rgba(248,250,252,0.72))] p-6 shadow-2xl shadow-slate-200/40 backdrop-blur-xl dark:bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.12),transparent_30%),radial-gradient(circle_at_bottom_right,rgba(245,158,11,0.10),transparent_34%),linear-gradient(135deg,rgba(15,23,42,0.88),rgba(2,6,23,0.82))] dark:shadow-black/20 md:p-8">
          ...
          <h1 className="mt-4 text-3xl font-black tracking-tight text-foreground md:text-4xl">Gestión de float y disponibilidad</h1>
              ...
              <Table>
                <TableHeader className="bg-muted/30">
                  <TableRow className="border-border/5 hover:bg-transparent">
                    <TableHead className="pl-8 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Gestor</TableHead>
                    <TableHead className="py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Contacto</TableHead>
                    <TableHead className="py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Estado</TableHead>
                    <TableHead className="py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground text-right">Float</TableHead>
                    <TableHead className="py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground text-right">Efectivo</TableHead>
                    <TableHead className="py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground text-right">Total recargado</TableHead>
                    <TableHead className="pr-8 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground text-right">Acción</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {agents.map((agent) => {
                    ...
                    return (
                      <TableRow key={agent.id} className="border-border/5 hover:bg-muted/30">
                        <TableCell className="pl-8 py-4">
                          <p className="text-sm font-black text-foreground">{agent.name}</p>
                        ...
                        <TableCell>
                          <div className="flex flex-col gap-2">
                            <Badge className={`w-fit rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] ${agent.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground'}`}>
                              {agent.is_active ? 'Activo' : 'Inactivo'}
                            </Badge>
                            {lowFloat && (
                              <Badge className="w-fit rounded-full bg-amber-100 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-amber-700">
                                Bajo float
                              </Badge>
                            )}
                          </div>
                        ...
                        <TableCell className="text-right">
                          <p className="text-base font-black text-foreground">{formatCurrency(agent.balance)}</p>
                        </TableCell>
                        <TableCell className="text-right">
                          <p className="text-base font-black text-foreground">{formatCurrency(agent.cash_balance || 0)}</p>
                        </TableCell>
                        <TableCell className="text-right">
                          <p className="text-base font-black text-foreground">{formatCurrency(agent.topup_total || 0)}</p>
                          {agent.last_topup_at && (
        ...
        <Dialog open={successOpen} onOpenChange={setSuccessOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader className="items-center text-center">
              ...
              <DialogTitle className="text-2xl font-black text-foreground">Recarga confirmada</DialogTitle>
            </DialogHeader>
            <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/10 p-6 text-center">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-700">Monto recargado</p>
              <p className="mt-2 text-4xl font-black text-foreground">{formatCurrency(successAmount)}</p>
            </div>
            <Button className="w-full rounded-2xl font-black" onClick={() => setSuccessOpen(false)}>
              Cerrar
            </Button>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl font-black text-foreground">Recargar saldo a gestor</DialogTitle>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <Label htmlFor="topup-amount" className="text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">
                  Monto a recargar
                </Label>
                <Input
                  id="topup-amount"
              ...
              {selectedAgent && (
                <div className="rounded-2xl border border-border/10 bg-background/70 p-4">
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Saldo actual</p>
                  <p className="mt-2 text-2xl font-black text-foreground">{formatCurrency(selectedAgent.balance)}</p>
                </div>
              )}
            </div>
            <DialogFooter>
              <Button variant="outline" className="rounded-xl font-black" onClick={() => setConfirmOpen(false)}>
                Cancelar
              </Button>
              <Button
                className="rounded-xl font-black"
        ...
        <Dialog open={finalConfirmOpen} onOpenChange={setFinalConfirmOpen}>
          <DialogContent className="max-w-md">
            <DialogHeader className="text-center">
              ...
              <DialogTitle className="text-xl font-black text-foreground">Confirmar recarga</DialogTitle>
              <DialogDescription className="text-sm font-semibold text-muted-foreground">
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-border/10 bg-background/70 p-4 text-center">
                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Gestor</p>
                <p className="mt-2 text-lg font-black text-foreground">{selectedAgent?.name}</p>
              </div>
              <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-center">
                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-700 dark:text-emerald-300">Monto</p>
                <p className="mt-2 text-3xl font-black text-foreground">{formatCurrency(parseFloat(topUpAmount) || 0)}</p>
              </div>
              {errorMessage && (
    ...
    return (
      <div className="space-y-8">
        <section className="rounded-[2rem] border border-border/10 bg-card/50 p-6 shadow-xl shadow-black/5 backdrop-blur-xl md:p-8">
          ...
          <h1 className="mt-4 text-3xl font-black tracking-tight text-foreground md:text-4xl">Mi billetera operativa</h1>
          <p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-muted-foreground">
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <Table>
                <TableHeader className="bg-muted/30">
                  <TableRow className="border-border/5 hover:bg-transparent">
                    <TableHead className="pl-8 py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Operación</TableHead>
                    <TableHead className="py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Monto</TableHead>
                    <TableHead className="py-4 text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">Detalle</TableHead>
                    ...
                    transactions.map((transaction) => (
                      <TableRow key={transaction.id} className="border-border/5 hover:bg-muted/30">
                        <TableCell className="pl-8">
                          <Badge className={`rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] ${transaction.amount >= 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-[2rem] border border-border/10 bg-card/50 p-6 shadow-xl shadow-black/5 backdrop-blur-xl md:p-8">
        ...
        <h1 className="mt-4 text-3xl font-black tracking-tight text-foreground md:text-4xl">Saldos por moneda y actividad</h1>
        <p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-muted-foreground">
        ...
        <CardContent className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-3">
          {currencySnapshots.map((snapshot) => (
            <div key={snapshot.currency} className="rounded-[1.75rem] border border-border/10 bg-background/70 p-5">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">{snapshot.currency}</p>
              <p className="mt-3 text-2xl font-black text-foreground">{formatCurrency(snapshot.available, snapshot.currency)}</p>
              <div className="mt-4 space-y-2 text-xs font-semibold text-muted-foreground">
                <div className="flex items-center justify-between">
                  <span>Reservado</span>
                  <span className="font-black text-foreground">{formatCurrency(snapshot.reserved, snapshot.currency)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Total bruto</span>
                  <span className="font-black text-foreground">{formatCurrency(snapshot.gross, snapshot.currency)}</span>
                </div>
              </div>
            </div>
          ))}
          ...
          {clientTransfers.length === 0 ? (
            ...
            clientTransfers.slice(0, 12).map((transfer) => (
              <div key={transfer.id} className="rounded-[1.5rem] border border-border/10 bg-background/70 p-4">
                ...
                <p className="mt-4 text-lg font-black text-foreground">{formatCurrency(transfer.amount, transfer.currency)}</p>
              </div>
            ))
          )}
        </CardContent>
